...
const handlePrepareReport = async () => {
  ...
  const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/reportSignature/check-report-approved/${year}/${month}`);
    
  if (response.data.exists) {
    Swal.fire({
      title: 'Report Already approved!',
      text: 'A report for this month has already been approved.',
      icon: 'info'
    });
    ...
    Swal.fire({
      title: 'Are you sure?',
      text: 'Do you want to approve and sign this report?',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        try {
          await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/reportSignature/approve-report`, {
            year,
            month,
            approvedBy: {
  ...
  <h1>Get stock report</h1>
    ...
    <div className="stock-history-container">
      <div className="stock-history-header" >
        ...
        <button className='verify-requisition' onClick={handlePrepareReport}>Approve Report</button>
