...
const LogisticRequestForm = () => {
  ...
  const fetchRequests = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/UserRequest/pending-requisition`);
...
const handleVerifySubmit = async () => {
   if (!isSigned) {
      Swal.fire({
        ...
        text: 'You must sign before verifying this requisition.',
        icon: 'warning',
        confirmButtonText: 'OK',
        customClass: {
          popup: 'custom-swal',
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
    await axios.put(`${process.env.REACT_APP_BACKEND_URL}/api/UserRequest/verified/${selectedRequest._id}`, { 
     ...
     toast.success('requisition verified successfully')
    ...
    toast.error('Failed to verify requisition')
      ...
      <div className="close-requisiton-form">
       <div>
        ...
        <label className='request-close-btn' onClick={() => setSelectedRequest(null)}><FaTimes /></label>
         ...
         <>
  {/* Your JSX content */}
  <ToastContainer position="top-right" autoClose={6000} hideProgressBar={false} newestOnTop={false} closeOnClick pauseOnFocusLoss draggable pauseOnHover />
</>
      </div>
