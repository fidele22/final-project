...
const ForwardedRequests = () => {
  ...
  const fetchForwardedRequests = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/LogisticRequest/pending-item-order`);
  ...
  const handleVerifySubmit = async (e) => {
    ...
    const confirmReject = await Swal.fire({
      ...
      if (confirmReject.isConfirmed) {
        try {
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/LogisticRequest/verifiedItemOrder/${selectedRequest._id}`,{
      verifiedBy: {
